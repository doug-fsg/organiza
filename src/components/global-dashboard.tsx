...
export function GlobalDashboard({ currentUser }: GlobalDashboardProps) {
  ...
  const metrics = useMemo(() => {
    ...
    const completedSubtasks = mainTasks.reduce((acc, task) => {
      return acc + task.subtasks.filter(s => s.status === SubtaskStatus.APPROVED).length
    }, 0)
  ...
  const userMetrics = useMemo(() => {
    ...
    return users.map(user => {
      ...
      const completedSubtasks = userSubtasks.filter(s => 
        s.status === SubtaskStatus.APPROVED
      ).length
  ...
  const calculateTaskProgress = (subtasks: any[]) => {
    ...
    const completed = subtasks.filter(s => s.status === SubtaskStatus.APPROVED).length
          ...
          <Card>
            <CardContent className="flex items-center p-6">
              <div className="flex items-center space-x-2">
                ...
                <div>
                  <p className="text-2xl font-bold">{Math.round(metrics.taskCompletionRate)}%</p>
